...
const firstDirection = {
  ...
  chance:"desc",relevance:"desc",listed:"desc",closing:"asc",type:"asc",status:"asc"
};

function applyOverrides(list){
  return list.map(j => ({...j, status: statusOverrides[j.id] || j.status}));
}
function persistOverride(id,status){
...
function closingSoon(j){
  const d=parseDate(j.closing);if(!d)return false;
  const now=new Date();
...
function filteredRows(){
  ...
  if(q)r=r.filter(j=>[j.role,j.employer,j.location,j.pay,j.career,j.employment,j.arrangement,j.status].join(" ").toLowerCase().includes(q));
  if(filters.has("new"))r=r.filter(j=>String(j.new).includes("NEW"));
...
function sortValue(j,key){
  ...
  if(key==="new")return String(j.new).includes("NEW")?1:0;
...
function render(){
  ...
  table.innerHTML=`<thead><tr>
    ...
    ${sh("Location / distance","distance")}${sh("Rate","rate")}
    ${sh("Employment","employment")}${sh("Work arrangement","arrangement")}
    ${sh("Chance","chance")}${sh("Relevance","relevance")}
    ${sh("Listed","listed")}${sh("Closing","closing")}
    ${sh("Type","type")}${sh("Status","status")}
    <th>Actions</th><th>Source</th>
  </tr></thead><tbody>`+r.map(j=>`<tr>
    <td>${esc(j.rank)}</td><td>${esc(j.new)}</td>
    ...
    <td><b>${esc(j.pay)}</b></td><td>${esc(j.employment)}</td><td>${esc(j.arrangement)}</td>
    <td>${esc(j.chance)}</td>
    <td><b>${esc(j.relevance_label)}</b><div>${esc(j.career)}</div></td>
    <td>${esc(j.listed)}</td><td>${esc(j.closing)}</td>
...
async function init(){
  document.getElementById("tabs").innerHTML=TABDEFS.map(([label,key])=>`<button class="tab ${key==="Dashboard"?"active":""}" data-tab="${esc(key)}" onclick="setTab('${esc(key)}')">${esc(label)}</button>`).join("");
  try{
    const res=await fetch("tracker.json",{cache:"no-store"});
    if(!res.ok)throw new Error(`HTTP ${res.status}`);
    const data=await res.json();
    const parts=await Promise.all((data.files||[]).map(async f=>{
    ...
    document.getElementById("updated").textContent=dataUpdatedAt?`Tracker data updated ${new Date(dataUpdatedAt).toLocaleString()}`:"Tracker data loaded";
    render();
  }catch(err){
    document.getElementById("updated").textContent="Could not load jobs.json";
    document.getElementById("tbl").innerHTML=`<tr><td class="error">Could not load tracker data: ${esc(err.message)}</td></tr>`;
  }
}
init();